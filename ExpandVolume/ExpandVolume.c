...
*/
int MountVolTemp (HWND hwndDlg, char *volumePath, int *driveNo, Password *password)
{
	...
	*driveNo = GetLastAvailableDrive ();
	...
	if (MountVolume (hwndDlg, *driveNo, volumePath, password, FALSE, TRUE, &mountOptions, TRUE, FALSE) < 1)
	{
		*driveNo = -3;
...
int ExtendFileSystem (HWND hwndDlg , char *lpszVolume, Password *pVolumePassword, uint64 newDataAreaSize)
{
	char szVolumeGUID[128];
	int driveNo = -1;
	char rootPath[] = "A:\\";
	...
	nStatus=MountVolTemp(hwndDlg, lpszVolume, &driveNo, pVolumePassword);
...
*/
static int ExpandVolume (HWND hwndDlg, char *lpszVolume, Password *pVolumePassword, uint64 newHostSize, BOOL initFreeSpace)
{
	...
	nStatus = ReadVolumeHeader (FALSE, buffer, pVolumePassword, &cryptoInfo, NULL);
	...
	if (initFreeSpace)
	{
		...
		nStatus = FormatNoFs (startSector, num_sectors, dev, cryptoInfo, FALSE);
	...
	while ( !cryptoInfo->LegacyVolume )
	{
			...
			)
		{
			...
			nStatus = WriteRandomDataToReservedHeaderAreas (dev, cryptoInfo, newDataAreaSize, !backupHeader, backupHeader);
			if (nStatus != ERR_SUCCESS)
				goto error;
		}
...
error:
	...
	if (nStatus == ERR_SUCCESS)
	{
		nStatus = ExtendFileSystem (hwndDlg, lpszVolume, pVolumePassword, newDataAreaSize);
	}
...
void __cdecl volTransformThreadFunction (void *pExpandDlgParam)
{
	...
	nStatus = ExpandVolume (hwndDlg, (char*)pParam->szVolumeName, pParam->pVolumePassword,
		pParam->newSize, pParam->bInitFreeSpace );
