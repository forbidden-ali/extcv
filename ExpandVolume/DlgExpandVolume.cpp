	...
	case WM_INITDIALOG:
		{
			...
			{
				sprintf (szTemp,"Please specify the new size of the TrueCrypt volume (must be at least %I64u KB larger than the current size).",TC_MINVAL_FS_EXPAND/1024);
			}
...
void ExpandVolumeWizard (HWND hwndDlg, char *lpszVolume)
{
	...
	case 1:
	case 2:
		MessageBoxW (hwndDlg, L"A TrueCrypt system volume can't be expanded.", lpszTitle, MB_OK|MB_ICONEXCLAMATION);
		goto ret;
	}
	...
	while (TRUE)
	{
		...
		if (!AskVolumePassword (hwndDlg, &VolumePassword, "ENTER_NORMAL_VOL_PASSWORD", FALSE))
		{
			goto ret;
		}
		...
		nStatus = OpenVolume (&expandVol, lpszVolume, &VolumePassword, FALSE, bPreserveTimestamp, FALSE);
	...
	nStatus=MountVolTemp(hwndDlg, lpszVolume, &driveNo, &VolumePassword);
	...
	if ( bIsDevice && bIsLegacy && volFSType != EV_FS_TYPE_NTFS )
	{
		MessageBoxW (hwndDlg,
			...
			L"is unsupported.\n"
			L"Note that expanding the TrueCrypt volume itself is not neccessary\n"
	...
	if (!bIsDevice && hostSize != volSize ) {
		...
		if (MessageBoxW (hwndDlg, L"Warning: The container file is larger than the TrueCrypt volume area. The data after the TrueCrypt volume area will be overwritten.\n\nDo you want to continue?", lpszTitle, YES_NO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
			goto ret;
	}
	...
	case EV_FS_TYPE_NTFS:
		break;
	case EV_FS_TYPE_FAT:
		if (MessageBoxW (hwndDlg,L"Warning: The TrueCrypt volume contains a FAT file system!\n\nOnly the TrueCrypt volume itself will be expanded, but not the file system.\n\nDo you want to continue?",
			lpszTitle, YES_NO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
			goto ret;
		break;
	default:
		if (MessageBoxW (hwndDlg,L"Warning: The TrueCrypt volume contains an unknown or no file system!\n\nOnly the TrueCrypt volume itself will be expanded, the file system remains unchanged.\n\nDo you want to continue?",
			lpszTitle, YES_NO|MB_ICONQUESTION|MB_DEFBUTTON2) == IDNO)
			goto ret;
	}
	...
	while (1)
	{
		...
		if ( newVolumeSize > TC_MAX_VOLUME_SIZE )
		{
			...
			swprintf(szTmp,L"Maximum TrueCrypt volume size of %I64u TB exceeded!\n",TC_MAX_VOLUME_SIZE/BYTES_PER_TB);
			MessageBoxW (hwndDlg, szTmp,lpszTitle, MB_OK | MB_ICONEXCLAMATION );
			if (bIsDevice)
